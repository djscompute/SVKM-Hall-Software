...
function HallPricing({ sessions, setHallData }: Props) {
  ...
  }>({ sessionName: null, categoryName: null });
  ...
  function addPriceCategory(sessionName: string) {
    if (!nameOfSessionToAddCategory.categoryName && nameOfSessionToAddCategory.sessionName)
      return;
    setHallData((prev) => {
      const modifiedSessionsArray = prev.sessions?.map((session) => {
        const newCategory = {
  ...
  function handleCategoryNameChange(
    ...
    setHallData((prev) => {
      const modifiedSessionsArray = prev.sessions?.map((session) => {
        if (session.name === sessionName) {
          const modifiedPriceArray = session?.price?.map((item) => {
            if (item.categoryName === oldCategoryName) {
              return {
                ...item,
  ...
  return (
    <div className="about-hall flex justify-between bg-gray-200 w-[60%] md:w-[90%] lg:w-full py-5 px-7 rounded-lg">
      <div className="flex flex-col gap-3 mb-5 w-full">
        <h2 className="font-bold text-xl mb-3 text-center">Pricing For sessions</h2>
        <div className="flex flex-col w-full">
          {sessions?.map((eachSession) => (
            <div className="flex flex-col w-full items-center" key={eachSession.name}>
                ...
                <table id="session-pricing-table" className="w-full">
                  <thead>
                    <tr>
                      ...
                      {eachSession.price?.map((eachCatObject) => (
                        <th key={eachCatObject.categoryName}>
                          {editingCategoryName.sessionName === eachSession.name &&
                          editingCategoryName.categoryName === eachCatObject.categoryName ? (
                            <input
                              ...
                              onBlur={() => {
                                ...
                                setEditingCategoryName({ sessionName: null, categoryName: null });
                              }}
                              onKeyDown={(e) => {
                                if (e.key === "Enter") {
                                  handleCategoryNameChange(
                            ...
                            <span
                              onClick={() => {
                                setEditingCategoryName({
                                  ...
                                  categoryName: eachCatObject.categoryName,
                                });
                                setTempCategoryName(eachCatObject.categoryName);
                              }}
                          ...
                          <button
                            ...
                            onClick={() =>
                              removePriceCategory(eachSession.name, eachCatObject.categoryName)
                            }
                          >
                            X
                          </button>
                  ...
                  <tbody>
                    <tr>
                      ...
                      {eachSession.price?.map((eachCatObject) => (
                        <td key={eachCatObject.categoryName}>
                          <input
                            ...
                            onChange={(e) => {
                              ...
                              setHallData((prev) => {
                                const modifiedSessionsArray = prev.sessions?.map((a) => {
                                  if (a.name == eachSession.name) {
                                    const modifiedPriceArray = a?.price?.map((item) => {
                                      if (item.categoryName === eachCatObject.categoryName) {
                                        return {
                                          ...item,
