...
import { EachHallAdditonalFeaturesType, EachHallType } from "../../types/Hall.types";
...
const HallAdditionalFeatures = ({ additionalFeatures, setHallData }: Props) => {
  const [modalData, setModalData] = useState<EachHallAdditonalFeaturesType[]>(additionalFeatures);
  const [modal, setModal] = useState(false);
  const [editedFeatures, setEditedFeatures] = useState<EachHallAdditonalFeaturesType[]>(modalData);
  const [newItem, setNewItem] = useState<EachHallAdditonalFeaturesType>({ heading: "", desc: "" });
                ...
                <ul>
                  {editedFeatures.map((item, index) => (
                    <li key={index} className="flex items-center justify-between">
                      <span className="p-4">{item.heading}</span>
